...
func change_project() -> void:
	...
	for i in range(layers.size() - 1, -1, -1):
		...
		var layer_container = load("res://src/scenes/animation/ui/timeline/LayerButton.tscn").instance()
		layer_container.i = i
		...
		for j in range(frames.size()): # Create Cel buttons
			var cel_button = load("res://src/scenes/animation/ui/timeline/CelButton.tscn").instance()
			cel_button.frame = j
			cel_button.layer = i
